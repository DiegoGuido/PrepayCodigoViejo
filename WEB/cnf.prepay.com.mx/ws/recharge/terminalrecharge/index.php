 ...
 * Purpose           : Terminal Recharge
 * Module			 : Global
 * Story			 : Terminal Recharge
 */
		...
		$setupInfo1 = getValidDevice($conn, $input); 
			...
			if(!$setupInfo1["error"]){
				...
				$reply["message"] = $setupInfo1["message"];            
					...
					$setupInfo2 = getTerminalRecharge($conn, $input); 
					//var_dump($setupInfo2);
					if(!$setupInfo2["error"]){
						$reply["status"] = true;
						$reply["message"] = $setupInfo2["message"];
						//$reply["balance"] = $setupInfo2["balance"];
						$reply["bal_ter"] = array_values($setupInfo2["balance"])[0];
					}else{
						$reply["status"] = false;
						$reply["message"] = $setupInfo2["message"];						
					}							
...
function grabJson($txtJson){
	...
	$txtJson = str_replace("\r\n","",$txtJson);
	$fp = fopen('log_ter_rec.txt', 'a');//opens file in append mode.
